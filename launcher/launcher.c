...
#define APP_TEXT_HEIGHT ((int)(launcher.height * 0.015))
...
void draw_launcher(cairo_t *cr, int width, int height) {
    ...
    int x, y;
    for (int i = 0; i < app_count; ++i) {
        x = (i % apps_per_row) * (app_icon_size + app_padding) + extra_padding + app_icon_size / 2;
        y = (i / apps_per_row) * (app_icon_size + APP_TEXT_HEIGHT + app_padding) + app_padding + app_icon_size / 2;
